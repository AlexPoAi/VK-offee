...
- ✅ Статус системы
- 🧠 Приём входящих задач в Telegram inbox
- 🎨 Приём дизайн-задач и фотографий в Telegram inbox
- 🤖 Отдельный `Codex mode` для рабочих задач вне RAG
...
- `monitor_bot.py` не должен использовать тот же `TELEGRAM_BOT_TOKEN`, что и продуктовый бот. Для него выделяется отдельный `MONITOR_BOT_TOKEN`.
...
```bash
TELEGRAM_ALLOWED_CHAT_ID=ваш_личный_chat_id
...
- `TELEGRAM_ALLOWED_CHAT_ID` ограничивает task/design-функции только вашим личным чатом.
- `TELEGRAM_TASK_INBOX` задаёт папку, куда бот будет складывать входящие задачи и фото.
- `CODEX_RUNTIME_ROOT` задаёт живое хранилище `session-card`, `task-card`, `artifacts`, `outbox` для `Codex mode`.
...
- `/codex` - Перейти в отдельный `Codex mode`
- `/rag` - Вернуться в режим базы знаний
- `/task <текст>` - Сохранить входящую задачу в inbox
- `/design <текст>` - Сохранить дизайн-задачу в inbox

## Разделение режимов

Бот теперь должен работать в двух отдельных режимах:

- `RAG mode` — обычные вопросы по базе знаний;
- `Codex mode` — рабочие задачи, фото, follow-up и отдельные агентные сессии.

Важно:

- `RAG mode` остаётся режимом по умолчанию;
- `Codex mode` включается только явно через кнопку `🤖 Codex` или `/codex`;
- возврат обратно делается через `📚 База знаний` или `/rag`;
- это разделение нужно, чтобы не ломать действующий контур поиска по базе знаний.

## 24/7 вход через Telegram

Если бот запущен на VPS через `systemd`, то он продолжает принимать:

- текстовые задачи;
- дизайн-запросы;
- фотографии для дальнейшей обработки;
- обычные RAG-вопросы по базе знаний.
- отдельные сообщения в `Codex mode`, которые materialize в `PACK-codex-runtime/runtime`.

### Что будет, если компьютер выключен

- Если бот и intake живут на VPS: **ничего критичного**, Telegram-вход продолжит работать 24/7.
- Если обработка завязана на локальный Codex на вашем компьютере: сами задачи будут приниматься, но их глубокая локальная обработка начнётся только когда локальная среда снова будет доступна.

То есть правильная схема такая:
- VPS = всегда включённый входной слой;
- локальный Codex = глубокая работа, когда нужен desktop/workspace контекст;
- дальше можно добавить cloud-worker, чтобы часть задач закрывалась вообще без включённого компьютера.
