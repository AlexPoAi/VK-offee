...
| PARK.COMM.030 | draft/source | 2026-04-27 | constantnine, ValerON, Александр | Telegram | Вопрос по скамейкам и урнам для СПОЗУ; рекомендован минимальный объём лавочек и урн | draft-response | `04-communications/PARK.COMM.030-spozu-benches-urns-question-april27 (...).md` |
...
2. Ответить проектировщикам по `PARK.COMM.030`: заложить минимально лавочки в зоне благоустройства / выхода в парк и урны рядом с лавочками и у входной группы
3. 28.04.2026 подать `PARK.DOC.038` в `ГУП РК "Вода Крыма"` и взять входящий номер / отметку о приёме
4. 29.04.2026 зарегистрироваться в личном кабинете Крымэнерго
5. Дождаться ответа администрации Бахчисарая по трубе
6. Запросить письменную фиксацию причины задержки АГО от ЛУКС
7. Запросить письменное разграничение ДС1 vs ДС3
8. Консультация юриста по оригиналу П240
