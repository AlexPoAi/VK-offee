...
Этот метод живёт не отдельно от intake-качества:
если `xlsx/csv/google sheet` читаются неполно, агент обязан сначала
починить/уточнить materialization layer, а не строить решение на обрезанных данных.
...
- нельзя считать вход "пустым", пока не проверен фактический data-range исходного файла
